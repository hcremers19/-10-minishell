...
/*   Updated: 2022/06/24 15:29:56 by hcremers         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	// ft_putstr_fd("Maillon reçu dans delone : ", 1); ft_putstr_fd(lst->name, 1); ft_putchar_fd(10, 1);
	if (!lst || !del)
		return ;
	(del)(lst->name);
	lst->name = NULL;
...
void	ft_env_lstfreenull(t_env *env_list)
{
	...
	while (env_list->next)
	{
		if (!env_list->next->name && !env_list->next->content)
		{
			...
			ft_putstr_fd(tmp_elmt->name, 1); ft_putchar_fd(10, 1);
